 ...
 */
 ...
 public class RTSPConnection {
     ...
     public RTSPResponse readRTSPResponse() throws IOException, RTSPException {
         try {
             ...
             while ((line = controlReader.readLine()) != null && !line.trim().isEmpty()) {
                 this.lastResponse = this.lastResponse + line;
                 ...
                 if (data.length == 1 && line.contains("RTSP")) { // # FOR SOME REASON IT IS NOT WORKING WILL COME BACK TO THIS
                     ...
                     // String currentSessionCode = response.getHeaderValue("SESSION");
                     // if (currentSessionCode != null && !data[1].equals(currentSessionCode)) {
             ...
             throw new RTSPException(e +" " +this.lastResponse);
         }
     }
     ...
     private String generateRequest(String operation) {
         ...
         return request;
         // setup - > play ;
         // if the play throws an error then the response used for the next play fails
     }
